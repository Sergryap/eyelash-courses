...
async def send_main_menu_answer(api: TgApi, event: TgEvent):
        ...
        if event.user_reply.split('_')[2] == 'c':
            ...
            if user_instance.phone_number:
                ...
                await api.send_message(
                    ...
                    msg=dedent(text))
            return 'PHONE'
    elif event.callback_query and event.user_reply.split(':')[0] == 'c':
        return await handle_course_info(api, event)
    return 'MAIN_MENU'


async def answer_arbitrary_text(api: TgApi, event: TgEvent):
    admin_msg = f'''
...
async def entry_user_to_course(api: TgApi, event: TgEvent, user, course):
    text = f'''
...
async def send_courses(api: TgApi, event: TgEvent, courses, msg1, msg2, msg3, /, *, back):
    i = 0
...
async def start(api: TgApi, event: TgEvent):
    start_buttons = ['/start']
    msg = 'MENU:'
...
async def main_menu_handler(api: TgApi, event: TgEvent):
...
async def handle_course_info(api: TgApi, event: TgEvent):
    """ Отправка сообщения о конкретном курсе """

    if event.callback_query and event.user_reply.split(':')[0] == 'c':
...
async def enter_phone(api: TgApi, event: TgEvent):
    user_instance = await Client.objects.async_get(telegram_id=event.chat_id)
    course_pk = api.redis_db.get(f'tg_{event.chat_id}_current_course')
    course = await Course.objects.async_get(pk=course_pk)

    # если номер существует
    if event.callback_query and event.user_reply.split('_')[0] == 'phone':
    ...
    elif event.callback_query and event.user_reply == 'admin_msg':
        await api.send_message(
            chat_id=event.chat_id,
            msg=f'_{event.first_name}, введите и отправьте ваше сообщение:_',
            parse_mode='Markdown'
